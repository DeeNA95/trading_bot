...
from train import get_best_device, prepare_data, create_reward_function
from environment.trading_env import TradingEnvironment

def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Test resuming training from a checkpoint')
    
    parser.add_argument('--checkpoint_path', type=str, default='models/checkpoint_2_checkpoint.pt',
...
def main():
    ...
    # Load the model
    agent.load_models(args.checkpoint_path.replace('_checkpoint.pt', ''), load_optimizer=True)
    ...
    agent.save_models(save_path.replace('_checkpoint.pt', ''), save_optimizer=True, save_memory=True)
