...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Visualize reward components from a trained model"
    )
    ...
    parser.add_argument(
        "--log_dir", type=str, default="logs", help="Directory containing logs"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def load_reward_components(model_name, log_dir):
    ...
    if not os.path.exists(components_dir):
        raise FileNotFoundError(
            f"Reward components directory not found: {components_dir}"
        )
...
def visualize_components(
    episodes, all_components, model_name, output_dir, moving_avg_window
):
    ...
    component_dict = {
        key: [comp.get(key, 0) for comp in all_components] for key in component_keys
    }
...
def main():
    ...
    try:
        ...
        visualize_components(
            episodes, all_components, args.model_name, output_dir, args.moving_avg
        )
