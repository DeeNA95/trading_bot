...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run multiple training jobs in parallel"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="parallel_training_results",
        help="Directory to save training results",
    )
    parser.add_argument(
        "--log_dir",
        type=str,
        default="parallel_logs",
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all jobs
        future_to_job = {
            executor.submit(run_training_job, i, config, args.log_dir, args.timeout): (
                i,
                config,
            )
        ...
        for future in as_completed(future_to_job):
            ...
            try:
                ...
                # Print progress
                print(
                    f"Completed job {job_id+1}/{len(configs)}: {result['model_name']} "
                ...
                results.append(
                    {
                        ...
                        "model_name": config.get(
                            "model_name", f"parallel_job_{job_id}"
                        ),
                        "success": False,
                        "return_code": -1,
                        "duration": 0,
